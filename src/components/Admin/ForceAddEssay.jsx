...
function ForceAddEssay() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await addDoc(collection(db, 'Essays'), { // Changed collection to 'Essays'
                title,
                description,
            ...
            navigate('/admin'); // Redirect to homepage or essays list
        } catch (error) {
            console.error('Error adding essay:', error.message); // Updated error message
        }
    };

    return (
        <div className="addEssayContainer"> {/* Reusing the CSS class */}
            <h2>Add Your Essay</h2> {/* Updated header */}
            <form onSubmit={handleSubmit}>
                ...
                <button type="submit" className={`${styles.button} ${styles.buttonPrimary}`}>Submit Essay</button>
            </form>
        </div>
    );
}
